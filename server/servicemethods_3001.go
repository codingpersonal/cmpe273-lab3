...
func PutKey_3001(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	key := vars["key_id"]
	key_int,_ := strconv.Atoi(key)
	
	value:=vars["value"]
	map_3001[key_int] = value
	fmt.Println("putting the value in map");
	w.WriteHeader(200);
}

//Get request handling - to get one key
func GetKey_3001(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	key := vars["key_id"]
	key_int,_ := strconv.Atoi(key)
	fmt.Println(key_int);
...
func GetAllKeys_3001(w http.ResponseWriter, r *http.Request) {
